(ns marketentry.store
  "SSoT for the MLI market-entry compliance actor, behind a `Store`
  ...
  every prior cloud-itonami actor in this fleet uses.
    ...
    - `DatomicStore` -- backed by `langchain.db`, a Datomic-API-compatible
                        EAV store. Uses `langchain-store.core` (`ls/enc`,
                        `ls/dec*`, `ls/read-stream`, `ls/append-blob!`)
                        for the EDN-blob codec + event-log read/append
                        instead of hand-rolling its own `enc`/`dec*`
...
(defn demo-data
  ...
  eng-1 clean, eng-2 no-spec-basis jurisdiction (\"ATL\", fictitious),
  eng-3 fee-mismatch, eng-4 missing-RCCM-entity, eng-5
  missing-NIF-verification, eng-6 another clean case with a different
  fee shape."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems ML" :portal "DGMP-DSP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-rccm-entity? true :has-rccm-entity? true
             :requires-nif? true :nif-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MLI" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "DGMP-DSP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-rccm-entity? true :has-rccm-entity? true
             :requires-nif? true :nif-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "DGMP-DSP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "MLI" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "DGMP-DSP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-rccm-entity? true :has-rccm-entity? false
             :requires-nif? true :nif-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MLI" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "DGMP-DSP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-rccm-entity? true :has-rccm-entity? true
             ...
             :drafted? false :submitted? false
             :jurisdiction "MLI" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "DGMP-DSP e-procurement"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-rccm-entity? has-rccm-entity?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-rccm-entity?)  (assoc :engagement/requires-rccm-entity? requires-rccm-entity?)
    (some? has-rccm-entity?)       (assoc :engagement/has-rccm-entity? has-rccm-entity?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-rccm-entity? :engagement/has-rccm-entity?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-rccm-entity? (boolean (:engagement/requires-rccm-entity? m))
     :has-rccm-entity? (boolean (:engagement/has-rccm-entity? m))
