(ns marketentry.governor-contract-test
  ...
  Also serves as the integration test: it runs the compiled
  `marketentry.operation` StateGraph end-to-end -- both a commit path
  (`clean-intake-auto-commits`) and a hold path
  (`fabricated-jurisdiction-is-held` / the RCCM-entity/fee/NIF hold
  tests below), not just the individual node functions."
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems ML"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems ML" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest rccm-entity-missing-is-held-and-unoverridable
  (testing "missing Mali RCCM-registered entity -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:rccm-entity-missing} (-> (store/ledger db) last :basis)))
...
(deftest nif-unverified-is-held-and-unoverridable
  (testing "unverified NIF when required -> HARD hold"
    (let [[db actor] (fresh)
